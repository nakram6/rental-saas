...
export default function ItemsIndex({ auth }) {
    ...
    const [form, setForm] = useState({
        id: null, // null = create, not null = edit
        name: '',
        sku: '',
        description: '',
        rental_type: 'bulk',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await axios.put(`/api/items/${form.id}`, {
                    name: form.name,
                    sku: form.sku,
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Inventory Items</h2>}
        >
            <Head title="Items" />
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 bg-white border-b border-gray-200">
                            ...
                            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Quantity Total</label>
                                    <input
                                        type="number"
                                        name="qty_total"
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {items.map((item) => (
                                                <tr key={item.id}>
                                                    ...
                                                    <td className="px-4 py-2 whitespace-nowrap text-sm text-right text-gray-500">
                                                        ${Number(item.security_deposit).toFixed(2)}
                                                    </td>
